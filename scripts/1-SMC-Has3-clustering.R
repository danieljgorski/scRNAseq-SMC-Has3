...
lesion_wt <- CreateSeuratObject(
  counts = lesion_wt,
  project = "SMC_Has3_WT_lesion",
  min.cells = 3, min.features = 200
)
lesion_ko <- CreateSeuratObject(
  counts = lesion_ko,
...
obj_features <- SelectIntegrationFeatures(
...
obj <- IntegrateData(
  anchorset = obj_anchors,
  normalization.method = "SCT"
)
obj <- RunPCA(object = obj)
ElbowPlot(object = obj)
obj <- FindNeighbors(object = obj, dims = 1:17)
obj <- FindClusters(object = obj, resolution = 0.5)
obj <- RunUMAP(object = obj, dims = 1:17)
DimPlot(obj, label = T)
